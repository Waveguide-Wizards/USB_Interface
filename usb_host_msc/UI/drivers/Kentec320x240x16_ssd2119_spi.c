...
#include "UI/drivers/Kentec320x240x16_ssd2119_spi.h"
...
#define LCD_SSI_GPIO_PERIPH     SYSCTL_PERIPH_GPIOB
#define LCD_SSI_GPIO_BASE       GPIO_PORTB_BASE
#define LCD_SSI_CLK_CFG         GPIO_PB4_SSI2CLK
#define LCD_SSI_TX_CFG          GPIO_PB7_SSI2TX
#define LCD_SSI_CLK_PIN         GPIO_PIN_4
#define LCD_SSI_TX_PIN          GPIO_PIN_7
...
InitSPILCDInterface(uint32_t ui32SysClock)
{
    ...
    //
    // Unlock PF0 and PD7 pins
    //
    HWREG(LCD_RST_BASE + GPIO_O_LOCK) = GPIO_LOCK_KEY;
    HWREG(LCD_RST_BASE + GPIO_O_CR)   = 0xFF;
    HWREG(LCD_RST_BASE + GPIO_O_LOCK) = ~(GPIO_LOCK_KEY);

    HWREG(LCD_LED_BASE + GPIO_O_LOCK) = GPIO_LOCK_KEY;
    HWREG(LCD_LED_BASE + GPIO_O_CR)   = 0xFF;
    HWREG(LCD_LED_BASE + GPIO_O_LOCK) = ~(GPIO_LOCK_KEY);

    //
    // Change PA4 into GPIO output for LCD_SCS
    // Change PA5 into GPIO output for LCD_SDC
    // Change PD7 into GPIO output for LCD_RST
    // Change PF2 into GPIO output for LED backlight
    //
    ...
    //
    // Configure the pin muxing for SSI2 functions on port B4, and B7.
    // This step is not necessary if your part does not support pin muxing.
    // TODO: change this to select the port/pin you are using.
    //
    GPIOPinConfigure(0x00011C02); //LCD_SSI_TX_CFG
    ...
    // freescale SPI mode, master mode, 10 MHz SSI frequency, and 8-bit data.
